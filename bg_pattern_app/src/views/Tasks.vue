  ...
  <div class="p-4">
    ...
    <div class="flex w-full justify-between mb-4">
      <div class="inputs flex items-center h-10">
        <DateRangePicker v-model:startDate="startDate" v-model:endDate="endDate" @change="onDatesChange" />
        <Dropdown v-model="selectedEmployee" :options="employeeOptions" optionLabel="name" optionValue="id"
          placeholder="Все сотрудники" class="mr-4 w-64" @change="onEmployeeChange"
          :disabled="loading || !startDate || !endDate" />
      </div>
      <ExcelCreate :excelData="excelData" fileName="Отчет_по_задачам.xlsx" @export="handleExportSuccess"
        @error="handleExportError" :disabled="!filteredTasks.length" />
    </div>
    ...
    <!-- Таблица с данными -->
    <div v-else-if="filteredTasks && filteredTasks.length > 0" class="overflow-x-auto mt-3">
      <DataTable :value="filteredTasks" responsiveLayout="scroll" class="p-datatable-sm" :rowClass="rowClassFunction"
        stripedRows paginator :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]" @row-click="onRowClick"
